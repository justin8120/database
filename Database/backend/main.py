...
#資料庫連線設定
#請將user, password, db替換成你本機Database裡設定的資訊
def get_db_connection():
    return pymysql.connect(
        host='localhost',
        user='root',          #你的MariaDB帳號
        password='nfu',       #你的MariaDB密碼
        db='NFU_database',    #你資料庫名稱
        charset='utf8mb4',
        cursorclass=pymysql.cursors.DictCursor #讓撈出來的資料自動變成Dictionary
    )
...
class SalesOrderCreate(BaseModel):
    ...
    details: List[OrderDetailCreate] #允許包進多筆產品明細
...
def get_full_order_details():
    """
    撈取資料庫中已經做好的View_Full_Order_Details檢視表
    """
    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
